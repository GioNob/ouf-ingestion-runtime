...
def shapefile(directory,filename='assets.zip',x=13.77):
    def header(size):
        b=bytearray(size)
        struct.pack_into('>i',b,0,9994);struct.pack_into('>i',b,24,size//2)
        struct.pack_into('<ii',b,28,1000,1)
        struct.pack_into('<dddd',b,36,x,45.65,x,45.65)
        return b
    shp=header(128);struct.pack_into('>ii',shp,100,1,10);struct.pack_into('<idd',shp,108,1,x,45.65)
    ...
    with zipfile.ZipFile(directory/filename,'w',zipfile.ZIP_DEFLATED) as z:
