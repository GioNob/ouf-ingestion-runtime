...
public class ActivationRepository {
 ...
 @Transactional public void reconcile(PublishedActivation a){
  ...
  db.sql("update ouf_ingestion.ing_run r set state='FAILED',failure_code='ING_ACTIVATION_SUPERSEDED',updated_at=transaction_timestamp() from ouf_ingestion.ing_schedule s where r.schedule_id=s.schedule_id and s.source_id=:source and r.state='PREFLIGHT' and (s.lease_until is null or s.lease_until<=transaction_timestamp()) and (r.bundle_checksum<>s.publication_checksum or not s.publication_enabled)").param("source",a.sourceId()).update();
  db.sql("update ouf_ingestion.ing_partition p set state='FAILED' from ouf_ingestion.ing_run r where p.run_id=r.run_id and r.source_id=:source and r.state='FAILED' and r.failure_code='ING_ACTIVATION_SUPERSEDED' and p.state='PREFLIGHT'").param("source",a.sourceId()).update();
