...
public class OperationalAwarenessService {
  ...
  public List<Map<String,Object>> history(String source,OffsetDateTime since,OffsetDateTime until,int limit,TrustedAuthorizationContext.Context actor){
    return sql.sql("select run_id as job_ref,source_id as source_ref,state as run_state,failure_code as error_code,correlation_id,created_at as first_seen_at,updated_at as last_seen_at,retry_stage from ouf_ingestion.ing_run where tenant_id=:t and (cast(:s as text) is null or source_id=:s) and updated_at between :since and :until order by updated_at desc,run_id desc limit :n")
      .param("t",actor.tenantId()).param("s",source).param("since",since).param("until",until).param("n",limit).query().listOfRows();
  }
