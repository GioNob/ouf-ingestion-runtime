  ...
  public static PublishedActivation decode(Map<String,Object> envelope,String tenant,ObjectMapper json){
    try{
      ...
      if(once){if(sync.get("pollInterval")!=null)throw new IllegalArgumentException("ING_MANAGED_POLLING_FORBIDDEN");}
        ...
        Map<String,Object> policy=map(sync,"operationalPolicy");ZoneId.of(text(policy,"timeZone"));bounded(policy,"misfireToleranceSeconds",1,86400);bounded(policy,"sourceTimeoutSeconds",1,300);bounded(policy,"maxRetryAttempts",0,100);bounded(policy,"maxRetryElapsedSeconds",1,86400);bounded(policy,"incidentDedupWindowSeconds",1,86400);bounded(policy,"operationalRetentionDays",30,3650);
