...
public final class RunExecutionWorker {
  ...
  private final RunExecutionRepository runs;private final RuntimePorts.AdapterResolver adapters;private final CanonicalRecordPipeline pipeline;private final SchemaSurveillanceService schemas;
  public RunExecutionWorker(RunExecutionRepository runs,RuntimePorts.AdapterResolver adapters,CanonicalRecordPipeline pipeline,SchemaSurveillanceService schemas){this.runs=runs;this.adapters=adapters;this.pipeline=pipeline;this.schemas=schemas;}
  public Optional<UUID> executeOne(String worker){runs.completeDrained();var claimed=runs.claim(worker,Duration.ofMinutes(2));if(claimed.isEmpty())return Optional.empty();var claim=claimed.orElseThrow();try{ExecutionBundle bundle=runs.bundle(claim.runId());var context=runs.context(claim.runId());AdapterSpi adapter=adapters.resolve(bundle);verifyAdapter(adapter,bundle);boolean exhausted=false;try(AdapterSpi.RecordCursor cursor=adapter.open(bundle,new AdapterSpi.Checkpoint(claim.checkpoint()))){for(int count=0;count<100;count++){var next=cursor.next();if(next.isEmpty()){exhausted=true;break;}AdapterSpi.SourceRecord record=next.orElseThrow();var command=new CanonicalRecordPipeline.Command(claim.runId(),claim.partitionKey(),record.ordinal(),1,adapter.adapterId(),bundle,record,context.correlationId(),cursor.checkpoint().value(),Map.of("ordinal",record.ordinal()));var observation=schemas.observe(claim.runId(),bundle,record,context.correlationId());var result=observation.outcome().blocking()?pipeline.reject(command,"ING_SCHEMA_"+observation.outcome().name()):pipeline.process(command);if(!result.succeeded()){String code=observation.outcome().blocking()?"ING_SCHEMA_"+observation.outcome().name():"ING_RECORD_QUARANTINED";runs.pause(claim,code);log("INGESTION_RUN_PAUSED",claim,context,code);return Optional.of(claim.runId());}}}if(exhausted)runs.inputExhausted(claim);else runs.release(claim);runs.completeDrained();log(exhausted?"INGESTION_INPUT_EXHAUSTED":"INGESTION_BATCH_COMPLETED",claim,context,null);return Optional.of(claim.runId());}catch(RuntimeException e){String code=safe(e);runs.pause(claim,code);var context=runs.context(claim.runId());log("INGESTION_RUN_PAUSED",claim,context,code);return Optional.of(claim.runId());}}
