...
public class ReplayWorker {
  ...
  public boolean executeOne(String worker){var found=repository.claim(worker,Duration.ofMinutes(5));if(found.isEmpty())return false;var c=found.orElseThrow();try{var receipt=executor.execute(c.replayId(),c.quarantineId(),c.targetBundleRef(),c.correlationId());repository.succeeded(c,receipt);metrics.replay("succeeded");LOG.atInfo().addKeyValue("event","REPLAY_SUCCEEDED").addKeyValue("replayId",c.replayId()).addKeyValue("quarantineId",c.quarantineId()).addKeyValue("correlationId",c.correlationId()).log("Ingestion replay completed");}catch(RuntimeException failure){repository.failed(c,"ING_REPLAY_EXECUTION_FAILED",Duration.ofMinutes(1));metrics.replay("retry");LOG.atWarn().addKeyValue("event","REPLAY_RETRY").addKeyValue("replayId",c.replayId()).addKeyValue("quarantineId",c.quarantineId()).addKeyValue("correlationId",c.correlationId()).addKeyValue("errorCode","ING_REPLAY_EXECUTION_FAILED").log("Ingestion replay scheduled for retry");}return true;}
}
