...
public class RunStateRepository {
  ...
  @Transactional public void recordFailure(String source,String code,AdapterSpi.ErrorClass errorClass,Duration retryAfter){
    ...
    int previous=sql.sql("select coalesce((select consecutive_failures from ouf_ingestion.source_health where source_id=:s),0)").param("s",source).query(Integer.class).single();
    int failures=previous+1;long base=Math.min(900,5L<<Math.min(failures-1,7));long jitter=Math.floorMod(Objects.hash(source,safe(code),failures),Math.max(1,(int)(base/5)+1));long delay=base+jitter;
    ...
    boolean open=failures>=CIRCUIT_THRESHOLD;
    sql.sql("insert into ouf_ingestion.source_health(source_id,status,consecutive_failures,circuit_open_until,retry_not_before,last_error_code) values(:s,:st,:n,case when :o then transaction_timestamp()+interval '15 minutes' end,transaction_timestamp()+(:d*interval '1 second'),:c) on conflict(source_id) do update set status=:st,consecutive_failures=:n,circuit_open_until=case when :o then transaction_timestamp()+interval '15 minutes' end,retry_not_before=transaction_timestamp()+(:d*interval '1 second'),recovery_probe_owner=null,recovery_probe_until=null,last_error_code=:c,updated_at=transaction_timestamp()")
      .param("s",source).param("st",open?"PAUSED":"DEGRADED").param("n",failures).param("o",open).param("d",delay).param("c",safe(code)).update();
  }
