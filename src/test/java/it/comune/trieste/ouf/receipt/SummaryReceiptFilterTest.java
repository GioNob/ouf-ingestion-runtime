...
class SummaryReceiptFilterTest {
  ...
  List<JsonNode> vectors()throws Exception{var result=new ArrayList<JsonNode>();for(String file:List.of("/operational-receipts.json","/incident-receipts.json"))result.addAll(new ObjectMapper().readValue(getClass().getResourceAsStream(file),new com.fasterxml.jackson.core.type.TypeReference<List<JsonNode>>(){}));return result;}
  LocalAuthorization engine(String owner,boolean grant,long version,String operation)throws Exception{
    var engine=new LocalAuthorization(clock,Duration.ofSeconds(300));String cap="ouf."+owner+".operations."+operation;
    ...
    TestAuthorization.install(engine,new PolicyBundle("bundle",version,Instant.EPOCH,List.of(new CapabilityDescriptor(cap,"READ",operation.equals("incidents")?"operations.incident.read":"operations.status.read",Set.of(PrincipalContext.ActorType.HUMAN))),grants));return engine;
  }
  int call(JsonNode vector,String body,String receipt,LocalAuthorization engine,Clock requestClock,String targetOwner,String tenant)throws Exception{
    ...
    var req=new MockHttpServletRequest("POST","/api/internal/v1/"+targetOwner+"/operations/"+vector.path("operation").asText("summary"));
  ...
  @Test void realLuaReceiptsRequireRoleFreshPolicyAndBodyIntegrity()throws Exception{
    for(var v:vectors()){
      String operation=v.path("operation").asText("summary");
      String owner=v.path("owner").asText(),body=v.path("body").asText(),receipt=v.path("receipt").asText();
      var auth=engine(owner,true,6,operation);
      assertEquals(v.path("hasRole").asBoolean()?204:403,call(v,body,receipt,auth,clock,owner,"tenant-a"));
      assertEquals(403,call(v,body,receipt,engine(owner,false,6,operation),clock,owner,"tenant-a"));
      assertEquals(403,call(v,body,receipt,engine(owner,true,7,operation),clock,owner,"tenant-a"));
