...
@SpringBootTest class RuntimeIssueGovernanceTest {
  ...
  @Test void quarantineHasTenantScopedIssueDurableReviewHumanClosureAndHistoricalRefs(){
    ...
    var actor=actor("tenant-a");assertThat(issues.search("OPEN",null,null,run,50,actor)).extracting(x->x.get("issue_id")).contains(issue);assertThat(issues.search("OPEN",null,null,run,50,actor("tenant-b"))).isEmpty();assertThat(issues.explain(issue,actor)).containsEntry("classification","ING_QUARANTINE_ITEM").containsEntry("recommendedAction","INSPECT_QUARANTINE_AND_REPROCESS").doesNotContainKey("payload_ref");
