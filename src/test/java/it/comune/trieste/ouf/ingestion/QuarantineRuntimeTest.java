...
@SpringBootTest class QuarantineRuntimeTest {
  ...
  @Test void payloadReferenceIsNotExposedAndReplayIsAuditedAgainstTrustedContext(){UUID run=run(),attempt=attempt(run);UUID q=quarantine.quarantine(run,attempt,"native-person-key","ING_MAPPING_NOT_APPLICABLE","object://secret-payload","evidence://safe","corr-q");Map<String,Object> view=quarantine.get(q);assertThat(view).doesNotContainKey("payload_ref").containsEntry("reason_code","ING_MAPPING_NOT_APPLICABLE");var actor=authorization.require(request("HUMAN_USER",Set.of("ingestion.quarantine.replay")),"ingestion.quarantine.replay",true);UUID replay=quarantine.requestReplay(q,"bundle://source/7",actor,"corr-r");assertThat(replay).isNotNull();assertThat(quarantine.get(q)).containsEntry("state","REPLAY_REQUESTED");assertThat(sql.sql("select tenant_id||':'||authorization_decision_ref from ouf_ingestion.audit_event where resource_id=:i").param("i",q.toString()).query(String.class).single()).isEqualTo("tenant-a:decision://42");}
  ...
  private UUID run(){UUID id=UUID.randomUUID();sql.sql("insert into ouf_ingestion.ing_run(run_id,source_id,bundle_id,bundle_version,bundle_checksum,mode,state,correlation_id) values(:i,:s,'b','1','sha256:x','REPLAY','RUNNING','corr')").param("i",id).param("s","source-"+id).update();return id;}
