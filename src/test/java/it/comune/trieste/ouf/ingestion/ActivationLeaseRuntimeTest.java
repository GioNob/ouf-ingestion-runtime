...
class ActivationLeaseRuntimeTest {
 ...
 @AfterEach void releaseFixtureSchedules(){db.sql("update ouf_ingestion.ing_schedule set state='DISABLED' where source_id like 'lease-%' or source_id like 'version-%'").update();}
 ...
 @Test void publishedRetryBudgetPausesAfterMaximumAndSamePublicationCannotResetIt(){String source="lease-retry-"+UUID.randomUUID();var e=PublishedActivationTest.envelope(source,false,1);var b=PublishedActivation.map(e,"bundle");PublishedActivation.map(PublishedActivation.map(b,"syncProfile"),"operationalPolicy").put("maxRetryAttempts",0);PublishedActivationTest.hash(b);e.put("checksum",b.get("checksum"));var a=PublishedActivation.decode(e,"tenant-a",PublishedActivationTest.JSON);activations.reconcile(a);var c=state.claimDue("worker",Duration.ofMinutes(2)).orElseThrow();state.activationFailed(c,null,new RunCoordinator.Failure("ING_GATEWAY_503",AdapterSpi.ErrorClass.TRANSIENT_SOURCE,null));activations.reconcile(a);assertThat(db.sql("select state from ouf_ingestion.ing_schedule where source_id=:s").param("s",source).query(String.class).single()).isEqualTo("PAUSED");assertThat(state.claimDue("retry",Duration.ofMinutes(2))).isEmpty();}
 @Test void preflightRetryReusesRunAndPersistsSchedulerMisfire(){
  String source="lease-timeline-"+UUID.randomUUID();var a=PublishedActivation.decode(PublishedActivationTest.envelope(source,false,1),"tenant-a",PublishedActivationTest.JSON);activations.reconcile(a);
  db.sql("update ouf_ingestion.ing_schedule set next_run_at=transaction_timestamp()-interval '5 minutes' where source_id=:s").param("s",source).update();
  var first=state.claimDue("first",Duration.ofMinutes(2)).orElseThrow();UUID run=state.createPreflightRun(first,a.execution(),"corr-first");
  assertThat(db.sql("select count(*) from ouf_ingestion.operational_incident where run_id=:r and event_type='SCHEDULER_MISFIRE'").param("r",run).query(Integer.class).single()).isOne();
  state.activationFailed(first,run,new RunCoordinator.Failure("ING_GATEWAY_503",AdapterSpi.ErrorClass.TRANSIENT_SOURCE,null));
  assertThat(state.run(run).get("state")).isEqualTo("RETRY_WAIT");
  db.sql("update ouf_ingestion.ing_schedule set next_run_at=transaction_timestamp()-interval '1 second' where source_id=:s").param("s",source).update();
  var second=state.claimDue("second",Duration.ofMinutes(2)).orElseThrow();
  assertThat(state.createPreflightRun(second,a.execution(),"corr-second")).isEqualTo(run);
  state.completePreflight(second,run);
  assertThat(db.sql("select count(*) from ouf_ingestion.operational_incident where run_id=:r and error_code='ING_GATEWAY_503'").param("r",run).query(Integer.class).single()).isOne();
  assertThat(db.sql("select lifecycle_state from ouf_ingestion.operational_incident where run_id=:r and error_code='ING_GATEWAY_503'").param("r",run).query(String.class).single()).isEqualTo("RECOVERING");
 }
