...
@SpringBootTest class DurableAckTest {
  ...
  @Test void restartCheckpointAdvancesBeforeAckButWatermarkWaitsForCompletePrefix(){
    ...
    var c1=repository.claim("w1",Duration.ofMinutes(1)).orElseThrow();assertThat(c1.handoffId()).isEqualTo(first);
    var c2=repository.claim("w2",Duration.ofMinutes(1)).orElseThrow();assertThat(c2.handoffId()).isEqualTo(second);
    repository.acknowledge(second,"w2","receipt-2");assertThat(watermarks(run)).isZero();
    repository.acknowledge(first,"w1","receipt-1");
  ...
  private UUID stage(UUID r,long n){return repository.stage(new DurablePipelineRepository.StageCommand(r,"default",n,"object-"+n,1,"managed-tabular-v1","1","bundle:1","idem-"+r+'-'+n,Map.of("row",n),Map.of("safe","evidence"),Map.of("ordinal",n),Map.of("ordinal",n)));}
